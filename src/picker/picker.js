...
function distPicker() {
    ...
    var dataArr = [];
    for (let i in districts['100000']) {
        let obj = {label: districts['100000'][i], value: i, children: []};
        for (let j in districts[i]) {
            let obj2 = {label: districts[i][j], value: j, children: []};

            for (let k in districts[j]) {
                let obj3 = {label: districts[j][k], value: k};
